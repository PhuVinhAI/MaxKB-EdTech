...
def hand_node(node, update_tool_map):
    ...
    if node.get("type") == "ai-chat-node":
        ...
        node_data["skill_tool_ids"] = [
            update_tool_map.get(tool_id, tool_id) for tool_id in skill_tool_ids
        ]
...
class ApplicationSerializer(serializers.Serializer):
    ...
    def to_application(application, workspace_id, user_id, update_tool_map, folder_id):
        ...
        return Application(
            ...
            file_upload_setting=application.get("file_upload_setting"),
            tool_ids=[
                update_tool_map.get(tool_id, tool_id)
                for tool_id in application.get("tool_ids", [])
            ],
            skill_tool_ids=[
                update_tool_map.get(tool_id, tool_id)
                for tool_id in application.get("skill_tool_ids", [])
            ],
            mcp_tool_ids=[
                update_tool_map.get(tool_id, tool_id)
                for tool_id in application.get("mcp_tool_ids", [])
            ],
        )
        ...
        try:
            ...
            if len(tool_id_list) > 0:
                ...
                tool_list = (
                    QuerySet(Tool)
                    .filter(
                        id__in=application.tool_ids
                        + application.mcp_tool_ids
                        + application.skill_tool_ids
                    )
                    .exclude(scope=ToolScope.SHARED)
                )
            ...
            for tool in tool_list:
                if tool.tool_type == ToolType.SKILL:
                    ...
                    if skill_file:
                        tool.code = base64.b64encode(skill_file.get_bytes()).decode(
                            "utf-8"
                        )
